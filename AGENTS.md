...
- `src/lib/`: framework-free helpers (markdown rendering in `markdown.ts` with syntax plugins in `markdown-*.ts`, HTML sanitising, code highlighting, HTML export, files, storage, backup merging, scroll mapping, text stats, pointer drags that outlive their element (`drag.ts`), `events.ts` for adding and removing a group of DOM listeners together).
